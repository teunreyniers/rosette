...
export async function svgzip(items) {
  const zip = new JSZip();
  for (let i = 0; i < items.length; i++) {
...
export async function pngzip(items) {
  const zip = new JSZip();
  for (let i = 0; i < items.length; i++) {
...
function createpng(svgtext) {
  return new Promise((resolve) => {
    ...
    const win = URL || window.URL || window.webkitURL || window;
...
export async function createPdf(items, layout) {
  ...
  FileSaver.saveAs(blob, `rosettes ${moment().format("YYYY-MM-DD h-mm")}.pdf`);
}
...
export async function createZip(items, type, layout) {
  ...
  FileSaver.saveAs(blob, `rosettes ${moment().format("YYYY-MM-DD h-mm")}.zip`);
}
